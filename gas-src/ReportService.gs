...
var SHIFT_DEFS = {
  ...
  /* n1 = เวรคลินิกนอกเวลา 1 (ค่าเริ่มต้น 06.00-08.00 น.) — เวรใหม่แยกจาก n โดยสิ้นเชิง มีเวลา/อัตรา
...
var ReportService = (function () {
  ...
  /* เหมือน repOTB1_ ทุกอย่าง แต่กรองเหลือแค่บทบาทเดียว (LA หรือ MT) — ปกติเวร บ1 (16.00-20.00) จัดอัตโนมัติ
  ...
  function repOTB1ByRole_(workload, year, month, laB, N, settings, role) {
    var rows = [];
    var tB1 = shiftTime_(settings, 'b1');
    for (var d = 1; d <= N; d++) {
      (laB[d] || []).forEach(function (pid) {
        var p = workload.people.filter(function (x) { return x.id === pid; })[0];
        if (p && roleOf_(p) === role) rows.push([d, p, tB1.s, tB1.e]);
      });
    }
    var h = '<table class="rep-tbl rep-tbl-sig"><thead>' + reportHeaderRows_(settings, year, month, 8) + SIG_HEAD_ROW_ + '</thead><tbody>';
    rows.forEach(function (row) {
      var d2 = row[0], p = row[1], s = row[2], e = row[3];
      h += '<tr><td class="c">' + dStr_(d2, year, month) + '</td><td class="l">' + fullN_(p) + '</td><td class="c">' + p.title + '</td>' +
  ...
  function renderReport(kind, year, month, pid) {
    ...
    if (kind === 'otsheetb1la') return { title: titleFor_(settings, kind), html: repOTB1ByRole_(workload, year, month, calendar.laB, calendar.N, settings, 'LA') };
    if (kind === 'otsheetb1mt') return { title: titleFor_(settings, kind), html: repOTB1ByRole_(workload, year, month, calendar.laB, calendar.N, settings, 'MT') };
