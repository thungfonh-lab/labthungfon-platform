...
const SHIFT_DEFS = {
  ...
  /* ตามฟีดแบ็ก: เลขกำกับเวรคลินิกนอกเวลาอิงเวลาเริ่ม ไม่ใช่ลำดับที่เพิ่มเข้าระบบ — n (07-08, เพิ่มก่อน)
     ชื่อว่า "2" เพราะเริ่มทีหลัง n1 (06-08, เพิ่มทีหลัง) ที่ชื่อว่า "1" เพราะเริ่มก่อน */
  n: { name: 'เวรคลินิกนอกเวลา 2 07-08', short: 'น', color: '#6b4e16', bg: '#ede0c4' },
  n1: { name: 'เวรคลินิกนอกเวลา 1 06-08', short: '<u>น</u>', color: '#b5541c', bg: '#f8dfc4' },
...
/* จัดลำดับคนให้ MT (รวม HT) อยู่กลุ่มเดียวกัน LA อยู่กลุ่มเดียวกัน แล้วเรียงตาม Priority ในแต่ละกลุ่ม
   ใช้กับ people ต้นทางของทุกรายงาน ให้แถวคนในรายงานเรียงลำดับเดียวกับตารางเวรและการ์ดติดต่อ */
function sortByGroupPriority(people) {
  return people.slice().sort((a, b) => {
    const ga = a.role === 'LA' ? 1 : 0, gb = b.role === 'LA' ? 1 : 0;
...
/* เหมือน repOTB1 ทุกอย่าง แต่กรองเหลือแค่บทบาทเดียว (LA หรือ MT) — ปกติเวร บ1 (16.00-20.00) จัดอัตโนมัติ
   ให้เฉพาะ LA (ดู laPool ใน businessService.generateSchedule) แต่ผ่านหน้า Override หัวหน้าใส่ MT
   เข้าเวรนี้ได้เหมือนกัน ใช้ฟังก์ชันเดียวสร้าง 2 รายงานแยกกันคนละแท็บ (otsheetb1la/otsheetb1mt)
   คนละหัวรายงาน/ชื่อรายงานตามปกติ — ไม่ใช่ตารางเดียวใส่ป้ายกลุ่มคั่น (หัวรายงานเพี้ยนไปจากรายงานอื่นตามฟีดแบ็ก) */
function repOTB1ByRole(workload, year, month, laB, N, settings, role) {
  const rows = [];
  const tB1 = shiftTime(settings, 'b1');
  for (let d = 1; d <= N; d++) {
    (laB[d] || []).forEach((pid) => {
      const p = workload.people.filter((x) => x.id === pid)[0];
      if (p && roleOf(p) === role) rows.push([d, p, tB1.s, tB1.e]);
    });
  }
  let h = '<table class="rep-tbl rep-tbl-sig"><thead>' + reportHeaderRows(settings, year, month, 8) + SIG_HEAD_ROW + '</thead><tbody>';
  rows.forEach((row) => {
    const d = row[0], p = row[1], s = row[2], e = row[3];
    h += '<tr><td class="c">' + dStr(d, year, month) + '</td><td class="l">' + fullN(p) + '</td><td class="c">' + p.title + '</td>' +
...
function payGroup(title, rows, shift, rateOvr, rates, showZero) {
  let h = '<tr class="grp"><td colspan="8">' + title + '</td></tr>'; let i = 0, sub = 0;
  rows.forEach((item) => {
    const has = item.days && item.days.length;
    ...
    i++;
    const amt = has ? item.days.reduce((s, d) => s + businessService.rateFor(rateOvr, rates, item.p, shift, d), 0) : 0;
    sub += amt;
    const rate = has ? businessService.rateFor(rateOvr, rates, item.p, shift, item.days[0]) : businessService.rateFor(rateOvr, rates, item.p, shift);
    h += '<tr><td class="c">' + i + '</td><td class="l">' + fullN(item.p) + '</td><td class="l">' + item.p.title + '</td><td class="c">' + rate + '</td>' +
      '<td class="l" style="white-space:normal">' + (has ? item.days.join(', ') : '—') + '</td><td class="c">' + (has ? item.days.length : 0) + '</td><td class="r">' + money(amt) + '</td><td></td></tr>';
  });
  return { h, sub };
}
...
function repPay(r, people, rateOvr, rates, settings, year, month) {
  ...
  let g = payGroup('วันที่ปฏิบัติงาน ' + paySectionLabel(settings, 'd0'), MT.map((p) => ({ p, days: (r[p.id] || {}).d0 })), 'd0', rateOvr, rates, true);
  h += g.h; tot += g.sub;
  g = payGroup('วันที่ปฏิบัติงาน ' + paySectionLabel(settings, 'd1'), MT.map((p) => ({ p, days: (r[p.id] || {}).d1 })), 'd1', rateOvr, rates, true);
  h += g.h; tot += g.sub;
  ...
  h += g.h; tot += g.sub;
  /* บ1 ปกติมีแต่ LA แต่ MT อาจได้ผ่าน Override เข้ามาด้วย (payGroup ข้ามคนที่ไม่มีวันอยู่แล้ว
     จึงใส่ MT.concat(LA) ได้โดยไม่กระทบคนที่ไม่มี บ1) — ไม่งั้นเงิน MT ที่ทำ บ1 จะตกหล่นจากใบเบิกเงิน */
  g = payGroup('วันที่ปฏิบัติงาน ' + paySectionLabel(settings, 'b1'), MT.concat(LA).map((p) => ({ p, days: (r[p.id] || {}).b1 })), 'b1', rateOvr, rates);
  h += g.h; tot += g.sub;
...
function repPayPerson(r, pid, people, rateOvr, rates, settings) {
  ...
  const rows = isLA ? [['ch', d.ch], ['b1', d.b1], ['n', d.n], ['n1', d.n1]] : [['ch', d.ch], ['b', d.b], ['n', d.n], ['n1', d.n1], ['d0', d.d0], ['d1', d.d1]];
...
function repShiftSummary(r, pid, people, rateOvr, rates, settings, stations, year, month) {
  ...
  if (!p || !d) return '<div style="padding:20px;color:var(--mut)">ไม่พบข้อมูล</div>';
  const isLA = roleOf(p) === 'LA';
  const shiftIds = isLA ? ['ch', 'b1', 'n', 'n1'] : ['ch', 'b', 'n', 'n1', 'd'];
  /* เวรข้ามบทบาทจาก Override (เช่น MT ได้ บ1, LA ได้ บ) ไม่อยู่ใน list ข้างบนตามปกติ — เพิ่มการ์ดให้
     เฉพาะตอนมีข้อมูลจริงเท่านั้น เหมือน repPayPerson ด้านบน */
...
function repTeamOverview(r, people, rateOvr, rates, year, month) {
  ...
  people.forEach((p) => {
    ...
    let row = '<tr><td class="l">' + fullN(p) + '</td><td class="l" style="font-size:11px">' + p.title + '</td>';
    ...
    shiftIds.forEach((s) => {
      ...
      if (days.length) row += '<div style="font-weight:700">' + days.length + '</div><div style="font-size:10px;color:var(--mut)">' + money(amt) + '</div>';
      else row += '<span style="color:#ccc">—</span>';
      row += '</td>';
    });
    row += '<td style="text-align:center;font-weight:700">' + rowTotal + '</td><td style="text-align:right;font-weight:700">' + money(rowAmt) + '</td></tr>';
...
const REPORT_TITLES = {
  ...
  otsheetb1la: 'บัญชีลงเวลาการปฏิบัติงานนอกเวลาราชการ 16.00-20.00 (ผู้ช่วยห้องปฏิบัติการ)',
  otsheetb1mt: 'บัญชีลงเวลาการปฏิบัติงานนอกเวลาราชการ 16.00-20.00 (นักเทคนิคการแพทย์)',
  otsheetd: 'บัญชีลงเวลาการปฏิบัติงานนอกเวลาราชการ  เวรดึก On call',
  nsheet: 'บัญชีลงเวลาการปฏิบัติงานนอกเวลาราชการ เวรคลินิกนอกเวลา 2',
...
async function renderReport(kind, year, month, pid) {
  ...
  const people = sortByGroupPriority(await dataService.getPeople());
  ...
  if (kind === 'otsheetb1la') return { title: titleFor(settings, kind), html: repOTB1ByRole(workload, year, month, calendar.laB, calendar.N, settings, 'LA') };
  if (kind === 'otsheetb1mt') return { title: titleFor(settings, kind), html: repOTB1ByRole(workload, year, month, calendar.laB, calendar.N, settings, 'MT') };
