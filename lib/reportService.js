...
/* เหมือน repOTB1 ทุกอย่าง แต่แยกเป็น 2 ตารางตามบทบาท (LA/MT) — ปกติเวร บ1 (16.00-20.00) จัดอัตโนมัติ
...
function repOTB1Split(workload, year, month, laB, N, settings) {
  const tB1 = shiftTime(settings, 'b1');
  const laRows = [], mtRows = [];
  for (let d = 1; d <= N; d++) {
    (laB[d] || []).forEach((pid) => {
      ...
      if (!p) return;
      (roleOf(p) === 'LA' ? laRows : mtRows).push([d, p, tB1.s, tB1.e]);
    });
  }
  function tableFor(rows, groupLabel) {
    if (!rows.length) return '';
    let t = '<div style="font-weight:700;font-size:13px;margin:14px 0 4px">' + groupLabel + '</div>' +
      '<table class="rep-tbl rep-tbl-sig"><thead>' + reportHeaderRows(settings, year, month, 8) + SIG_HEAD_ROW + '</thead><tbody>';
    rows.forEach((row) => {
      const d = row[0], p = row[1], s = row[2], e = row[3];
      t += '<tr><td class="c">' + dStr(d, year, month) + '</td><td class="l">' + fullN(p) + '</td><td class="c">' + p.title + '</td>' +
        '<td class="c">เวรบ่าย</td><td class="c">' + s + '</td><td class="sig-col"></td><td class="c">' + e + '</td><td class="sig-col"></td></tr>';
    });
    return t + '</tbody></table>';
  }
  const html = tableFor(laRows, 'กลุ่ม LA (ผู้ช่วยห้องปฏิบัติการ)') + tableFor(mtRows, 'กลุ่ม MT (นักเทคนิคการแพทย์)');
...
async function renderReport(kind, year, month, pid) {
  ...
  if (kind === 'otsheetb1split') return { title: titleFor(settings, kind), html: repOTB1Split(workload, year, month, calendar.laB, calendar.N, settings) };
