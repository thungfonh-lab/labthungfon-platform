...
function repOT(workload, year, month, scheduleAssign, N, settings) {
  const rows = [];
  for (let d = 1; d <= N; d++) {
    const a = scheduleAssign[d];
    if (!a) continue;
    if (businessService.isOff(year, month, d, [])) (a.ch || []).forEach((pid) => rows.push([d, pid, 'ch', '08.00', '16.00']));
    (a.b || []).forEach((pid) => rows.push([d, pid, 'b', '16.00', '00.00']));
  }
...
function repOTB1(workload, year, month, laB, N, settings) {
  const rows = [];
  for (let d = 1; d <= N; d++) { (laB[d] || []).forEach((pid) => rows.push([d, pid, 'b1', '16.00', '20.00'])); }
  let h = '<table class="rep-tbl rep-tbl-sig"><thead>' + reportHeaderRows(settings, year, month, 8) + SIG_HEAD_ROW + '</thead><tbody>';
  rows.forEach((row) => {
    const p = workload.people.filter((x) => x.id === row[1])[0];
    if (!p) return;
...
function repPay(r, people, rateOvr, rates, settings, year, month) {
  ...
  let g = payGroup('วันที่ปฏิบัติงาน ' + PAY_SECTION_LABEL.d0, MT.map((p) => ({ p, days: (r[p.id] || {}).d0 })), 'd0', rateOvr, rates);
  h += g.h; tot += g.sub;
  g = payGroup('วันที่ปฏิบัติงาน ' + PAY_SECTION_LABEL.d1, MT.map((p) => ({ p, days: (r[p.id] || {}).d1 })), 'd1', rateOvr, rates);
  h += g.h; tot += g.sub;
  g = payGroup('วันที่ปฏิบัติงาน ' + PAY_SECTION_LABEL.ch, MT.concat(LA).map((p) => ({ p, days: (r[p.id] || {}).ch })), 'ch', rateOvr, rates);
  h += g.h; tot += g.sub;
...
function repPayPerson(r, pid, people, rateOvr, rates, settings) {
  ...
  rows.forEach((pair) => {
    ...
    tot += amt;
    h += '<tr><td class="l">' + sDef.name + '</td><td class="c">' + rate + '</td><td class="l" style="white-space:normal">' + (days.join(', ') || '—') + '</td><td class="c">' + days.length + '</td><td class="r">' + money(amt) + '</td></tr>';
  });
...
function repOcClaim(pid, oncall, people, year, month) {
  ...
  let h = '<div style="text-align:center;font-size:14px;margin-bottom:10px">' +
...
async function renderReport(kind, year, month, pid) {
  ...
  if (kind === 'occlaim') {
    return { title: titleFor(settings, kind), html: header + repOcClaim(pid, await dataService.getOnCallRecords(year, month), people, year, month) };
  }
  if (kind === 'ocstats') {
  ...
  if (kind === 'otsheet') return { title: titleFor(settings, kind), html: repOT(workload, year, month, calendar.assign, calendar.N, settings) };
...
const EXPORT_CSS = '<style>table{border-collapse:collapse}th,td{border:1px solid #999;padding:4px 8px;font-family:"TH Sarabun New","Sarabun",sans-serif;font-size:14px}th{background:#F5F3EE;font-weight:bold;text-align:center}td.l{text-align:left}td.c{text-align:center}td.r{text-align:right}</style>';
