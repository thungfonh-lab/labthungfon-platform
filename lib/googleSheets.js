...
/** Clears every cached sheet/header read — must run at the START of every HTTP
 *  request (see api/index.js middleware). DataService.gs's original per-execution
 *  cache reset for free on each Apps Script invocation; on Vercel, a warm
 *  serverless instance can be reused across multiple unrelated HTTP requests, so
 *  without this a write in one request could leave a later request on the same
 *  (or another) warm instance reading stale cached rows — e.g. a manual override
 *  saving successfully but a calendar reload moments later still showing the old
 *  assignment. invalidateReadAllCache() alone only ever cleared the *writing*
 *  instance's copy, not every instance that might serve the next read. */
function resetCaches() {
  Object.keys(_headerCache).forEach((k) => delete _headerCache[k]);
  Object.keys(_readAllCache).forEach((k) => delete _readAllCache[k]);
}
