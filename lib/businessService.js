...
function dayTypeAllowsPerson(p, isWeekend) {
  const dt = p && p.dayTypes;
  if (!dt) return true;
...
const HALF_DAY_BLOCKS = { leaveAM: ['noCh', 'noN'], leavePM: ['noCh'] };

/** Ported from avOff()/avNo() in original.
 *  avOff = ว่างทั้งวันไหม (ลาครึ่งวันไม่นับ เพราะยังขึ้นเวรที่ไม่คาบเกี่ยวได้) */
function avOff(availability, pid, day, month, year) {
  return (availability[pid] || []).some((a) => a.day === day && a.mon === month && a.yr === year && (a.type === 'leave' || a.type === 'offAll'));
}
function avNo(availability, pid, day, month, year, type) {
  return (availability[pid] || []).some((a) => {
    if (a.day !== day || a.mon !== month || a.yr !== year) return false;
    if (a.type === 'leave' || a.type === 'offAll' || a.type === type) return true;
    return (HALF_DAY_BLOCKS[a.type] || []).indexOf(type) > -1;
  });
}
...
/** Ported from rateFor_(). ความเจาะจง: บุคคล > บทบาท > (ทุกคน) แล้วเสริมด้วยช่วงวันที่ */
function rateFor(rateOvr, rates, p, shift, day) {
  let best = null, score = -1;
  rateOvr.forEach((o) => {
    if (o.shift !== shift) return;
    if (o.pid) { if (o.pid !== p.id) return; }
    else if (o.role) { if (o.role !== roleOf(p)) return; }
    if (o.from && day && (day < o.from || (o.to && day > o.to))) return;
    const s = (o.pid ? 3 : (o.role ? 2 : 0)) * 10 + (o.from ? 1 : 0);
...
async function generateSchedule(year, month) {
  ...
  /* วันที่ขึ้นเวรได้รายบุคคล (person.dayTypes) — แบ่ง 2 กลุ่มตามโครงสร้างบล็อกเวรจริง:
     "วันทำการ" = เวรวันเดี่ยว จ-พฤ · "วันหยุด/สุดสัปดาห์" = บล็อกศุกร์ (ศ-ส-อา) + นักขัตฤกษ์
     (ศุกร์นับเป็นวันหยุดเพราะระบบจัด ศ-ส-อา เป็นบล็อกเดียวเสมอ) ตัวเช็คจริงอยู่ที่ dayTypeAllowsPerson */
  const peopleById = {};
  people.forEach((p) => { peopleById[p.id] = p; });
  const blockIsWeekend = (days) => dow(year, month, days[0]) === 5 || days.some((d) => isOffFn(d));
  const dayTypeAllows = (pid, days) => dayTypeAllowsPerson(peopleById[pid], blockIsWeekend(days));

  function canDuty(pid, days) {
    /* dayTypeAllows = ข้อจำกัดกลุ่มวันรายบุคคล — ครอบทั้ง pick() และ cross-lock อัตโนมัติ
       ส่วน fallback tier สุดท้ายของ pick (pool.slice()) ยังผ่อนให้เมื่อไม่มีใครว่างจริงๆ */
    if (!dayTypeAllows(pid, days)) return false;
    return days.every((d) => !avOff(availability, pid, d, month, year) &&
      ...
      !(!disCH && isOffFn(d) && avNo(availability, pid, d, month, year, 'noCh')));
  }
  ...
  const prevMonthRows = await dataService.getScheduleAssignments(prevY, prevM);
  ...
  let prevLastDayPid = null;
  if (prevMonthRows.length) {
    const prevN = dim(prevY, prevM);
    const prevUnflat = unflattenAssignments(prevMonthRows, prevN);
    const lastDay = prevUnflat.assign[prevN] || {};
    /* ด ที่ยกยอดข้ามเดือน = คนที่อยู่เวร บ วันสุดท้าย — อ่าน b ก่อน (ตามการสลับเวร) แล้ว fallback ไป d */
    const lastArr = (lastDay.b && lastDay.b.length) ? lastDay.b : lastDay.d;
  ...
  const prevLastDay = dim(prevY, prevM);
  if (dow(prevY, prevM, prevLastDay) === 5) {
    const lastFriBRow = prevMonthRows.find((r) => r.day === prevLastDay && r.shiftType === 'b');
    const friPid = lastFriBRow ? lastFriBRow.pid : prevLastDayPid;
    if (friPid) {
      let lockD = 1;
      while (lockD <= N && isOffFn(lockD)) {
  ...
  for (let day = 1; day <= N; day++) {
    ...
    if (!disMT) {
      ...
      const cands = cp.filter((p) => !avNo(availability, p.id, day, month, year, 'noN') && dayTypeAllows(p.id, [day]));
    ...
    if (!disLA) {
      if (settings.clinicLa === '' || settings.clinicLa === undefined || settings.clinicLa === null) {
        ...
        const laCands = laPool.filter((p) => {
          if (avNo(availability, p.id, day, month, year, 'noN')) return false;
        ...
        if (lp && lp.active && !avNo(availability, settings.clinicLa, day, month, year, 'noN')) clinicLa[day] = settings.clinicLa;
      }
    }
  }
  ...
  const prevB1All = prevMonthRows.filter((r) => r.shiftType === 'b1');
  ...
  if (!disB1) {
    for (let day2 = 1; day2 <= N; day2++) {
      ...
      if (la && laPerson && laPerson.active && !avNo(availability, la, day2, month, year, 'noB1')) laB[day2] = la;
    }
  }
  ...
  const postPassLog = runPostBalancePass(pool, assign, clinicMt, N, isOffFn, maxC, rateOvr, rates, people, year, month);
  ...
  await dataService.finalizeGenerateWrites(year, month, trailInfo, ruleViolations, overrides.concat(postPassLog));
...
function runPostBalancePass(pool, assign, clinicMt, N, isOffFn, maxC, rateOvr, rates, people, year, month) {
  ...
  for (let pass = 0; pass < maxPass; pass++) {
    ...
    /* วันที่ขึ้นเวรได้รายบุคคลของผู้รับ (un) — กันตัวปรับสมดุลย้ายเวรไปให้คนที่ตั้งค่าไม่รับกลุ่มวันนั้น
       (tier วันหยุดต้องรับ weekend ได้, tier วันทำการต้องรับ weekday ได้) */
    const unWeekendOK = dayTypeAllowsPerson(un.p, true);
    const unWeekdayOK = dayTypeAllowsPerson(un.p, false);
    ...
    const inFriBlock = (d) => {
      if (dow(year, month, d) === 5) return true;
      if (!isOffFn(d)) return false;
      let x = d;
      while (x >= 1 && isOffFn(x)) x--;
      if (x >= 1) return dow(year, month, x) === 5;
      return dow(year, month, 0) === 5;
    };

    for (let d1 = 1; d1 <= N && !swapped && unWeekendOK; d1++) {
      const a1 = assign[d1] || {};
    ...
    for (let d2 = 1; d2 <= N && !swapped && unWeekdayOK; d2++) {
      const a2 = assign[d2] || {};
    ...
    for (let d5 = 1; d5 <= N && !swapped && unWeekdayOK; d5++) {
      const a5 = assign[d5] || {};
...
function unflattenAssignments(rows, N) {
  const assign = {}, clinicMt = {}, clinicLa = {}, laB = {}, n1 = {};
  for (let d = 1; d <= N; d++) assign[d] = { ch: [], b: [], d: [] };
  rows.forEach((r) => {
    if (!r.pid) return;
  ...
  return { assign, clinicMt, clinicLa, laB, n1, N };
}
...
/* createBlankSchedule(year, month) — ตารางเปล่าสำหรับจัดเวรเองผ่าน override
   เขียน sentinel row 1 แถว (shiftType 'blank' — unflattenAssignments ไม่รู้จัก จึงไม่กระทบ grid/รายงาน)
   เพื่อให้เดือนนับเป็น 'draft' + trailInfo กลาง ๆ กันเดือนถัดไปอ่านค่าค้าง */
async function createBlankSchedule(year, month) {
  const N = dim(year, month);
  await dataService.clearSchedule(year, month);
  await dataService.saveScheduleAssignments(year, month, [{ day: 0, shiftType: 'blank', pid: '' }], 'draft');
  await dataService.finalizeGenerateWrites(year, month,
    { lastPid: null, trailingCount: 0, lastDow: dow(year, month, N), lastDay: N, lastDayPid: null, laB: null }, [], []);
  return { year, month, status: 'draft', blank: true };
}
...
async function loadCalendar(year, month) {
  ...
  const prevLastDayPidsForLoad = await (async () => {
    ...
    if (pr.length) {
      ...
      const lastDay = pu.assign[pN] || {};
      /* ด ที่ยกยอดข้ามเดือน = คนที่อยู่เวร บ วันสุดท้าย (บ 16-00 ต่อเนื่องเป็น ด 00-08 คืนนั้น)
         อ่านจาก b ก่อน เพื่อให้ตามการสลับเวร บ วันสุดท้ายหลังสร้างตาราง แล้วค่อย fallback ไป d */
      if (lastDay.b && lastDay.b.length) return lastDay.b;
      if (lastDay.d && lastDay.d.length) return lastDay.d;
    }
    const pt = await dataService.getTrailInfo(prevY2, prevM2);
    if (pt && pt.lastDayPid) return [pt.lastDayPid];
    return [];
  })();
  if (prevLastDayPidsForLoad.length) {
    const people2 = await dataService.getPeople();
    const activePrevLDP = prevLastDayPidsForLoad.filter((pid) => { const cp3 = byId(people2, pid); return cp3 && cp3.active !== false; });
  ...
  return {
    ...
    shiftTimes: (await dataService.getSettings()).shiftTimes || {}
  };
}
...
async function calculateWorkload(year, month) {
  ...
  const r = {};
  people.forEach((p) => { r[p.id] = { ch: [], b: [], b1: [], n: [], n1: [], d: [], d0: [], d1: [] }; });

  for (let d = 1; d <= N; d++) {
    const a = unflat.assign[d];
    if (a) {
      if (isOff(year, month, d, holidays)) (a.ch || []).forEach((pid) => { if (r[pid]) r[pid].ch.push(d); });
      (a.b || []).forEach((pid) => { if (r[pid]) r[pid].b.push(d); });
    }
    (unflat.laB[d] || []).forEach((pid) => { if (r[pid]) r[pid].b1.push(d); });
    (unflat.clinicMt[d] || []).forEach((pid) => { if (r[pid]) r[pid].n.push(d); });
    (unflat.clinicLa[d] || []).forEach((pid) => { if (r[pid]) r[pid].n.push(d); });
    (unflat.n1[d] || []).forEach((pid) => { if (r[pid]) r[pid].n1.push(d); });
  }
  oncall.forEach((o) => {
  ...
  people.forEach((p) => {
    ...
    r[p.id].n1.forEach((day) => { m += rateFor(rateOvr, rates, p, 'n1', day); });
...
async function approveBalanceSwap(year, month, ovId, unId, shift) {
  ...
  if (shift === 'b') {
    for (let d1 = N; d1 >= 1; d1--) { if (unflat.assign[d1] && (unflat.assign[d1].b || []).indexOf(ovId) > -1) { swapDay = d1; break; } }
  } else if (shift === 'ch') {
    for (let d2 = N; d2 >= 1; d2--) { if (unflat.assign[d2] && (unflat.assign[d2].ch || []).indexOf(ovId) > -1) { swapDay = d2; break; } }
  } else if (shift === 'b1') {
    for (let d3 = N; d3 >= 1; d3--) { if ((unflat.laB[d3] || []).indexOf(ovId) > -1) { swapDay = d3; break; } }
  } else if (shift === 'n') {
  ...
  await dataService.removeAssignment(year, month, swapDay, shift, ovId);
  await dataService.addAssignment(year, month, swapDay, shift, unId);
