...
async function generateSchedule(year, month) {
  ...
  /* === DEBUG TEMP: remove after deploy verified === */
  const _dbgPrevLastDay = dim(prevY, prevM);
  const _dbgFriRow = prevMonthRows.find((r) => r.day === _dbgPrevLastDay && r.shiftType === 'b');
  const _debug = {
    buildMarker: 'bd22f54-crossmonth-fix',
    prevY, prevM,
    prevLastDay: _dbgPrevLastDay,
    prevLastDow: dow(prevY, prevM, _dbgPrevLastDay),
    prevMonthRowsCount: prevMonthRows.length,
    friRowPid: _dbgFriRow ? _dbgFriRow.pid : null,
    prevLastDayPid,
    crossLock,
    day1: assign[1], day2: assign[2],
    day1InFriBlockCheck: { dow0: dow(year, month, 0), isOff1: isOffFn(1), isOff2: isOffFn(2) }
  };
  /* === END DEBUG === */
  ...
  return {
    ...
    postBalanceSwaps: postPassLog.length, ruleViolations, _debug
  };
}
