...
async function generateSchedule(year, month) {
  ...
  function pick(days) {
    ...
    let cands = pool.filter((p) => canDuty(p.id, days) && consec(p.id, days[0]) < maxC);
  ...
  while (dd <= N) {
    ...
    if (dow(year, month, dd) === 5) {
      ...
      const pid = pick(bl); bl.forEach((b) => setD(b, pid)); dd = x;
    } else if (isOffFn(dd)) {
  ...
  /* LA บ1 */
  const laMonPer = settings.laB === 'A' ? settings.laA : settings.laB2;
  const laWedPer = settings.laB === 'A' ? settings.laB2 : settings.laA;
  if (!disB1) {
...
function computeTrailInfo(N, assign, isOffFn) {
  ...
  return { lastPid, trailingCount, lastDay: N };
}
