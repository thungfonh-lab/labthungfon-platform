...
async function setAssignment(year, month, day, shiftType, pid) {
  const rows = await getScheduleAssignments(year, month);
  const existing = rows.filter((r) => r.day === day && r.shiftType === shiftType)[0];
  const status = rows.length ? (rows[0].status || 'draft') : 'draft';
  if (existing) {
    /* Sheets v4 values.update treats a literal null cell as "leave unchanged", unlike
       Apps Script's Range.setValues() which clears it — must send '' to actually clear. */
    await gs.updateRow(SHEETS.SCHEDULES, existing.__rowIndex, { year, month, day, shiftType, pid: pid || '', status });
  } else if (pid) {
...
module.exports = {
  ...
  getScheduleAssignments, getScheduleStatus, saveScheduleAssignments, setScheduleStatus, setAssignment,
