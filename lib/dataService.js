...
const THAI_LUNAR_ESTIMATED = [2574, 2575];
...
async function seedThaiHolidays(year) {
  ...
  return { added, hasLunarTable: !!THAI_LUNAR_HOLIDAYS[year], estimated: THAI_LUNAR_ESTIMATED.indexOf(year) > -1 };
}
...
async function clearSchedule(year, month) {
  const schRows = await getScheduleAssignments(year, month);
  if (schRows.length) await gs.deleteRowsBulk(SHEETS.SCHEDULES, schRows.map((r) => r.__rowIndex));

  const CLEAR_CATEGORIES = [TX_CATEGORY.OVERRIDE, TX_CATEGORY.RULE_VIOLATION, TX_CATEGORY.TRAIL_INFO];
  const txRows = (await gs.readAll(SHEETS.TRANSACTIONS)).filter((r) => r.year === year && r.month === month && CLEAR_CATEGORIES.indexOf(r.category) > -1);
  if (txRows.length) await gs.deleteRowsBulk(SHEETS.TRANSACTIONS, txRows.map((r) => r.__rowIndex));
...
async function getTransactionById(txId) {
  const rows = await gs.readAll(SHEETS.TRANSACTIONS);
  return rows.filter((r) => r.id === txId)[0] || null;
}
...
function isBefore(year, month, beforeYear, beforeMonth) {
  const y = Number(year), m = Number(month);
  return y < beforeYear || (y === beforeYear && m < beforeMonth);
}

/** นับแถวที่จะถูกลบถ้า archive จริง (preview ก่อนยืนยัน) */
async function previewArchive(beforeYear, beforeMonth) {
  const tx = (await gs.readAll(SHEETS.TRANSACTIONS)).filter((r) => isBefore(r.year, r.month, beforeYear, beforeMonth)).length;
  const sch = (await gs.readAll(SHEETS.SCHEDULES)).filter((r) => isBefore(r.year, r.month, beforeYear, beforeMonth)).length;
  return { transactions: tx, schedules: sch };
}

/** ลบข้อมูล Transactions + Schedules ที่เก่ากว่า beforeYear/beforeMonth ออกจาก sheet */
async function archiveOldData(beforeYear, beforeMonth) {
  const txRows = (await gs.readAll(SHEETS.TRANSACTIONS)).filter((r) => isBefore(r.year, r.month, beforeYear, beforeMonth));
  if (txRows.length) await gs.deleteRowsBulk(SHEETS.TRANSACTIONS, txRows.map((r) => r.__rowIndex));

  const schRows = (await gs.readAll(SHEETS.SCHEDULES)).filter((r) => isBefore(r.year, r.month, beforeYear, beforeMonth));
  if (schRows.length) await gs.deleteRowsBulk(SHEETS.SCHEDULES, schRows.map((r) => r.__rowIndex));
...
async function updateUser(userId, patch) {
  ...
  if (patch.email && String(patch.email).toLowerCase() !== String(rec.email).toLowerCase()) {
    const dupe = await findUserByEmailRaw(patch.email);
    if (dupe) throw new Error('มีบัญชีอีเมลนี้อยู่แล้ว: ' + patch.email);
  }
  const headers = await gs.getHeaders(SHEETS.USERS);
  const fields = ['name', 'email', 'role'];
  for (const f of fields) {
    if (patch[f] !== undefined && patch[f] !== '') await gs.updateCell(SHEETS.USERS, rec.__rowIndex, headers.indexOf(f) + 1, patch[f]);
  }
  return { userId, name: patch.name || rec.name, email: patch.email || rec.email, role: patch.role || rec.role };
}
...
module.exports = {
  ...
  getOnCallRecords, addOnCallRecord, deleteTransactionById, updateTransactionById, getTransactionById,
  getPayAdjustments, setPayAdjustment,
  getOverrides, addOverride, addOverridesBulk, clearAutoOverrides, clearSchedule,
  ...
  previewArchive, archiveOldData,
  listUsers, findUserByEmailRaw, findUserById, createUser, updateUser, setUserActive, resetUserPassword,
